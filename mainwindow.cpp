...
/*
Some notes on things I'd like to put into the program but haven't put on github
(yet)

Allow scripts to read/write signals from DBC files
allow scripts to load DBC files in support of the script - maybe the graphing
system too.
*/
...
MainWindow *MainWindow::getReference() { return selfRef; }
...
MainWindow::MainWindow(QWidget *parent)
  ...
  this->setWindowTitle("Savvy CAN V" + QString::number(VERSION) + " [Built " +
                       QString(__DATE__) + "]");

  model = new CANFrameModel(
      this); // set parent to mainwindow to prevent canframemodel to change
             // thread (might be done by setModel but just in case)
  ...
  settingsDialog =
      new MainSettingsDialog(); // instantiate the settings dialog so it can
                                // initialize settings if this is the first run
                                // or the config file was deleted.
  settingsDialog->updateSettings(); // write out all the settings. If this is
                                    // the first run it'll write defaults out.
  ...
  if (settings.value("Main/FontFixedWidth", false).toBool())
    sysFont = QFontDatabase::systemFont(
        QFontDatabase::FixedFont); // get default fixed width font
  else
  ...
  HorzHdr->setStretchLastSection(
      true); // causes the data column to automatically fill the tableview
  ...
  connect(ui->actionSetup, SIGNAL(triggered(bool)),
          SLOT(showConnectionSettingsWindow()));
  connect(ui->actionOpen_Log_File, &QAction::triggered, this,
          &MainWindow::handleLoadFile);
  connect(ui->actionGraph_Dta, &QAction::triggered, this,
          &MainWindow::showGraphingWindow);
  connect(ui->actionFrame_Data_Analysis, &QAction::triggered, this,
          &MainWindow::showFrameDataAnalysis);
  connect(ui->actionSave_Log_File, &QAction::triggered, this,
          &MainWindow::handleSaveFile);
  connect(ui->actionSave_Filtered_Log_File, &QAction::triggered, this,
          &MainWindow::handleSaveFilteredFile);
  connect(ui->actionLoad_Filter_Definition, &QAction::triggered, this,
          &MainWindow::handleLoadFilters);
  connect(ui->actionSave_Filter_Definition, &QAction::triggered, this,
          &MainWindow::handleSaveFilters);
  connect(ui->action_Playback, &QAction::triggered, this,
          &MainWindow::showPlaybackWindow);
  connect(ui->actionFlow_View, &QAction::triggered, this,
          &MainWindow::showFlowViewWindow);
  connect(ui->action_Custom, &QAction::triggered, this,
          &MainWindow::showFrameSenderWindow);
  connect(ui->actionExit_Application, &QAction::triggered, this,
          &MainWindow::exitApp);
  connect(ui->actionFuzzy_Scope, &QAction::triggered, this,
          &MainWindow::showFuzzyScopeWindow);
  connect(ui->actionRange_State_2, &QAction::triggered, this,
          &MainWindow::showRangeWindow);
  connect(ui->actionSave_Decoded_Frames, &QAction::triggered, this,
          &MainWindow::handleSaveDecoded);
  connect(ui->actionSave_Decoded_Frames_CSV, &QAction::triggered, this,
          &MainWindow::handleSaveDecodedCsv);
  connect(ui->actionSingle_Multi_State_2, &QAction::triggered, this,
          &MainWindow::showSingleMultiWindow);
  connect(ui->actionFile_Comparison, &QAction::triggered, this,
          &MainWindow::showComparisonWindow);
  connect(ui->actionDBC_Comparison, &QAction::triggered, this,
          &MainWindow::showDBCComparisonWindow);
  connect(ui->actionScripting_INterface, &QAction::triggered, this,
          &MainWindow::showScriptingWindow);
  connect(ui->actionPreferences, &QAction::triggered, this,
          &MainWindow::showSettingsDialog);
  connect(ui->actionFirmware_Update, &QAction::triggered, this,
          &MainWindow::showFirmwareUploaderWindow);
  connect(ui->actionDBC_File_Manager, &QAction::triggered, this,
          &MainWindow::showDBCFileWindow);
  connect(ui->actionFuzzing, &QAction::triggered, this,
          &MainWindow::showFuzzingWindow);
  connect(ui->actionUDS_Scanner, &QAction::triggered, this,
          &MainWindow::showUDSScanWindow);
  connect(ui->actionISO_TP_Decoder, &QAction::triggered, this,
          &MainWindow::showISOInterpreterWindow);
  connect(ui->actionSniffer, &QAction::triggered, this,
          &MainWindow::showSnifferWindow);
  connect(ui->actionMotorControlConfig, &QAction::triggered, this,
          &MainWindow::showMCConfigWindow);
  connect(ui->actionCapture_Bisector, &QAction::triggered, this,
          &MainWindow::showBisectWindow);
  connect(ui->actionSignal_Viewer, &QAction::triggered, this,
          &MainWindow::showSignalViewer);
  connect(ui->actionSave_Continuous_Logfile, &QAction::triggered, this,
          &MainWindow::handleContinousLogging);
  connect(ui->actionTemporal_Graph, &QAction::triggered, this,
          &MainWindow::showTemporalGraphWindow);
  connect(ui->actionCAN_Bridge, &QAction::triggered, this,
          &MainWindow::showCANBridgeWindow);

  // handlers fror interactions with the main can frame view table
  connect(ui->canFramesView, &QAbstractItemView::clicked, this,
          &MainWindow::gridClicked);
  connect(ui->canFramesView, &QAbstractItemView::doubleClicked, this,
          &MainWindow::gridDoubleClicked);
  ui->canFramesView->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(ui->canFramesView, &QAbstractItemView::customContextMenuRequested,
          this, &MainWindow::gridContextMenuRequest);

  connect(model, &CANFrameModel::updatedFiltersList, this,
          &MainWindow::updateFilterList);
  connect(CANConManager::getInstance(), &CANConManager::framesReceived, model,
          &CANFrameModel::addFrames);
  // new implementation for continuous logging
  connect(CANConManager::getInstance(), &CANConManager::framesReceived, this,
          &MainWindow::logReceivedFrame);

  connect(ui->cbInterpret, &QAbstractButton::toggled, this,
          &MainWindow::interpretToggled);
  connect(ui->cbOverwrite, &QAbstractButton::toggled, this,
          &MainWindow::overwriteToggled);
  connect(ui->cbPersistentFilters, &QAbstractButton::toggled, this,
          &MainWindow::presistentFiltersToggled);
  connect(ui->cbHideTx, &QAbstractButton::toggled, this,
          &MainWindow::hideTxToggled);
  connect(ui->listFilters, &QListWidget::itemChanged, this,
          &MainWindow::filterListItemChanged);
  connect(ui->listBusFilters, &QListWidget::itemChanged, this,
          &MainWindow::busFilterListItemChanged);

  connect(ui->btnCaptureToggle, &QAbstractButton::clicked, this,
          &MainWindow::toggleCapture);
  connect(ui->btnClearFrames, &QAbstractButton::clicked, this,
          &MainWindow::clearFrames);
  connect(ui->btnNormalize, &QAbstractButton::clicked, this,
          &MainWindow::normalizeTiming);
  connect(ui->btnFilterAll, &QAbstractButton::clicked, this,
          &MainWindow::filterSetAll);
  connect(ui->btnFilterNone, &QAbstractButton::clicked, this,
          &MainWindow::filterClearAll);
  connect(ui->btnExpandAll, &QAbstractButton::clicked, this,
          &MainWindow::expandAllRows);
  connect(ui->btnCollapseAll, &QAbstractButton::clicked, this,
          &MainWindow::collapseAllRows);

  connect(ui->tableSimpleSender, SIGNAL(cellChanged(int, int)), this,
          SLOT(onSenderCellChanged(int, int)));
  ...
  // Prevent annoying accidental horizontal scrolling when filter list is
  // populated with long interpreted message names
  ...
  ui->canFramesView->resizeRowToContents(
      0); // Resize the row to fit the contents so we get a proper height value
  qApp->processEvents();
  tickGUIUpdate(); // force a GUI refresh so that the row exists to measure
  normalRowHeight = ui->canFramesView->rowHeight(0);
  if (normalRowHeight == 0)
    normalRowHeight = 30; // should not be necessary but provides a sane number
                          // if something stupid happened.
  qDebug() << "normal row height = " << normalRowHeight;
  model->clearFrames();

  ui->canFramesView->verticalHeader()->setDefaultSectionSize(
      normalRowHeight); // Set the default height for all rows to the height
                        // that was calculated

  // connect(CANConManager::getInstance(),
  // CANConManager::connectionStatusUpdated, this,
  // MainWindow::connectionStatusUpdated);
  connect(CANConManager::getInstance(), SIGNAL(connectionStatusUpdated(int)),
          this, SLOT(connectionStatusUpdated(int)));

  // Automatically create the connection window so it can be updated even if we
  // never opened it.
  connectionWindow = new ConnectionWindow();
  connect(this, SIGNAL(suspendCapturing(bool)), connectionWindow,
          SLOT(setSuspendAll(bool)));

  // these either are unfinished/not working or are not for general use.
  // But,they exist so if you want to enable them and play with them then go for
  // it.
  ...
  frameSender->startSending(); // start the timer in the object so enabled
                               // things can send
...
void MainWindow::killEmAll() {
  ...
  // trying to kill this window can cause a fault to happen. It's closed last
  // just in case.
...
void MainWindow::exitApp() {
  this->close();
  QApplication::quit(); // forces the whole application to terminate when the
                        // main window is closed
}

// the close event can be trapped and ignored so put unsaved warnings in here so
// the user can abort the program closing if they forgot to save things.
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  for (int i = 0; i < dbcHandler->getFileCount(); i++) {
    ...
    if (file->getDirtyFlag()) {
      confirmDialog = QMessageBox::question(
          this, "Unsaved DBC",
          "DBC File:\n" + file->getFilename() +
...
void MainWindow::readSettings() {
  ...
  if (settings.value("Main/SaveRestorePositions", false).toBool()) {
    ...
    ui->canFramesView->setColumnWidth(
        0, settings.value("Main/TimeColumn", 150).toUInt()); // time stamp
    ui->canFramesView->setColumnWidth(
        1, settings.value("Main/IDColumn", 70).toUInt()); // frame ID
    ui->canFramesView->setColumnWidth(
        2, settings.value("Main/ExtColumn", 40).toUInt()); // extended
    ui->canFramesView->setColumnWidth(
        3, settings.value("Main/RemColumn", 40).toUInt()); // remote
    ui->canFramesView->setColumnWidth(
        4, settings.value("Main/DirColumn", 40).toUInt()); // direction
    ui->canFramesView->setColumnWidth(
        5, settings.value("Main/BusColumn", 40).toUInt()); // bus
    ui->canFramesView->setColumnWidth(
        6, settings.value("Main/LengthColumn", 40).toUInt()); // length
    ui->canFramesView->setColumnWidth(
        7, settings.value("Main/AsciiColumn", 50).toUInt()); // ascii
    // ui->canFramesView->setColumnWidth(8, settings.value("Main/DataColumn",
    // 225).toUInt()); //data
  }
...
/*
 * TODO: The way the frame timing mode is specified is DEAD STUPID. There
 * shouldn't be three boolean values for this. Instead switch it all to an ENUM
 * or something sane.
 */
void MainWindow::readUpdateableSettings() {
  ...
  model->setTimeFormat(
      settings.value("Main/TimeFormat", "MMM-dd HH:mm:ss.zzz").toString());
...
void MainWindow::processSenderCellChange(int line, int col) {
  ...
  case SIMP_COL::SC_COL_BUS: // Bus designation
    tempVal = Utility::ParseStringToNum(
        ui->tableSimpleSender->item(line, SIMP_COL::SC_COL_BUS)->text());
    if (tempVal < -1)
      tempVal = -1;
    if (tempVal >= numBuses)
      tempVal = numBuses - 1;
  ...
  case SIMP_COL::SC_COL_ID: // ID field
    tempVal = Utility::ParseStringToNum(
        ui->tableSimpleSender->item(line, SIMP_COL::SC_COL_ID)->text());
    if (tempVal < 0)
      tempVal = 0;
    ...
    if (tempData->frameId() > 0x7FF) {
      ...
      ui->tableSimpleSender->item(line, ST_COLS::SENDTAB_COL_EXT)
  ...
  case SIMP_COL::SC_COL_EXT:
    if (ui->tableSimpleSender->item(line, SIMP_COL::SC_COL_EXT)->checkState() ==
        Qt::Checked) {
      tempData->setExtendedFrameFormat(true);
    } else {
  ...
  case SIMP_COL::SC_COL_REM:
    if (ui->tableSimpleSender->item(line, SIMP_COL::SC_COL_REM)->checkState() ==
        Qt::Checked) {
  ...
  case SIMP_COL::SC_COL_DATA: // Data bytes
    for (int i = 0; i < 8; i++)
      tempData->payload().data()[i] = 0;

#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
    tokens = ui->tableSimpleSender->item(line, SIMP_COL::SC_COL_DATA)
                 ->text()
  ...
  case SIMP_COL::SC_COL_INTERVAL: // interval in ms
    ...
    QString trigger =
        ui->tableSimpleSender->item(line, SIMP_COL::SC_COL_INTERVAL)
            ->text()
            .toUpper();
    ...
    thisTrigger.ID =
        -1; // the rest of these being -1 means nothing has changed it
...
void MainWindow::updateConnectionSettings(QString connectionType, QString port,
                                          int speed0, int speed1) {
...
void MainWindow::disableAutoRowExpansion() { rowExpansionActive = false; }
...
void MainWindow::collapseAllRows() {
  ...
  if (goAhead) {
    for (int i = 0; i < numRows; i++)
      ui->canFramesView->setRowHeight(i, normalRowHeight);
...
void MainWindow::gridDoubleClicked(QModelIndex idx) {
  ...
  emit sendCenterTimeID(frame.frameId(),
                        frame.timeStamp().microSeconds() / 1000000.0);
}

void MainWindow::gridContextMenuRequest(QPoint pos) {
  QModelIndex idx = ui->canFramesView->indexAt(
      pos); // figure out where in the view we clicked (row, column)
  qDebug() << "Pos: " << pos << " Row :" << idx.row()
           << " Col: " << idx.column();
  ...
  if (idx.column() == 8) // we're over the DATA column
  {
    ...
    menu->addAction(tr("Add to a new graphing window"), this,
                    SLOT(setupAddToNewGraph()));
    menu->addSeparator();
    menu->addAction(tr("Add to latest graphing window"), this,
...
QString MainWindow::getSignalNameFromPosition(QPoint pos) {
  // there's a bit of an issue to solve. The data column is one big string even
  // if there are a number of signals in there. So, the basic idea is to find
  // out how tall the font is and where the user clicked within the cell. Then
  // find out which line that puts us over.
  QModelIndex idx = ui->canFramesView->indexAt(
      pos); // figure out where in the view we clicked (row, column)
...
uint32_t MainWindow::getMessageIDFromPosition(QPoint pos) {
  QModelIndex idx = ui->canFramesView->indexAt(
      pos); // figure out where in the view we clicked (row, column)
  QString idText =
      ui->canFramesView->model()->index(idx.row(), 1).data().toString();
...
void MainWindow::setupSendToLatestGraphWindow() {
  ...
  if (msg) {
    ...
    if (sig) {
      ...
      param.lineColor = QColor(QRandomGenerator::global()->bounded(160),
                               QRandomGenerator::global()->bounded(160),
                               QRandomGenerator::global()->bounded(160));
      param.lineWidth = 1;
      ...
      QString boxmsg =
          "Cannot find ID 0x" +
          QStringLiteral("%1").arg(param.ID, 3, 16, QLatin1Char('0')) +
          " in DBC message " + msg->name + ". Not adding graph.";
    ...
    QString boxmsg =
        "Cannot find ID 0x" +
        QStringLiteral("%1").arg(param.ID, 3, 16, QLatin1Char('0')) +
        " in DBC file(s). Not adding graph.";
    msgbox.setText(boxmsg);
    msgbox.exec();
  }
}
...
void MainWindow::overwriteToggled(bool state) {
  if (state) {
    QMessageBox::StandardButton confirmDialog;
    confirmDialog = QMessageBox::question(
        this, "Danger Will Robinson",
        "Enabling Overwrite mode will\ndelete your captured frames\nand "
        "replace them with one\nframe per ID.\n\nAre you ready to do that?",
        QMessageBox::Yes | QMessageBox::No);
    if (confirmDialog == QMessageBox::Yes) {
      model->setOverwriteMode(true);
    } else
      ui->cbOverwrite->setCheckState(Qt::Unchecked);
  } else {
    rowExpansionActive = false;
...
void MainWindow::hideTxToggled(bool state) {
  qDebug() << "Hide TX" << state;
  model->setHideTx(state);
  manageRowExpansion();
}
...
void MainWindow::updateFilterList() {
  ...
  for (filterIter = filters->begin(); filterIter != filters->end();
       ++filterIter) {
    /*QListWidgetItem *thisItem = */ FilterUtility::createCheckableFilterItem(
        filterIter.key(), filterIter.value(), ui->listFilters);
  }

  if (busFilters->isEmpty())
    return;

  for (filterIter = busFilters->begin(); filterIter != busFilters->end();
       ++filterIter) {
    /*QListWidgetItem *thisItem = */ FilterUtility::
        createCheckableBusFilterItem(filterIter.key(), filterIter.value(),
                                     ui->listBusFilters);
  }
  inhibitFilterUpdate = false;
}
...
void MainWindow::logReceivedFrame(CANConnection *conn,
...
void MainWindow::tickGUIUpdate() {
  ...
  if (model->needsFilterRefresh())
    updateFilterList();

  if (continuousLogging) {
    //            const QVector<CANFrame> *modelFrames =
    //            model->getListReference();
    //            FrameFileIO::writeContinuousNative(modelFrames,
    //            modelFrames->count() - rxFrames);
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    if (tempData) {
      ui->tableSimpleSender->item(i, SIMP_COL::SC_COL_COUNT)
          ->setText(QString::number(tempData->count));
    }
  }
...
void MainWindow::normalizeTiming() {
  ...
  emit framesUpdated(
      -2); // claim an all new set of frames because every frame was updated.
}
...
void MainWindow::handleLoadFile() {
  ...
  if (!loadResult) {
    ...
    {
      confirmDialog = QMessageBox::question(
          this, "Error Loading", "Do you want to salvage what could be loaded?",
          QMessageBox::Yes | QMessageBox::No);
      if (confirmDialog == QMessageBox::Yes) {
        loadResult = true;
      }
...
void MainWindow::handleDroppedFile(const QString &filename) {
  ...
  if (!loadResult) {
    ...
    {
      QMessageBox::StandardButton confirmDialog = QMessageBox::question(
          this, "Error Loading", "Do you want to salvage what could be loaded?",
          QMessageBox::Yes | QMessageBox::No);
      if (confirmDialog == QMessageBox::Yes) {
        loadResult = true;
...
void MainWindow::handleSaveFilters() {
  ...
  dialog.setDirectory(
      settings.value("Filters/LoadSaveDirectory", dialog.directory().path())
          .toString());
...
void MainWindow::handleLoadFilters() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    filename = dialog.selectedFiles()[0];
    // right now there is only one file type that can be loaded here so just do
    // it.
...
void MainWindow::handleSaveDecoded() { handleSaveDecodedMethod(false); }

void MainWindow::handleSaveDecodedCsv() { handleSaveDecodedMethod(true); }
...
void MainWindow::handleSaveDecodedMethod(bool csv) {
  ...
  QStringList filters;
  if (!csv)
    filters.append(QString(tr("Text File (*.txt *.TXT)")));
  else
    filters.append(QString(tr("CSV File (*.csv *.CSV)")));

  dialog.setDirectory(
      settings.value("FileIO/LoadSaveDirectory", dialog.directory().path())
          .toString());
  dialog.setFileMode(QFileDialog::AnyFile);
  dialog.setNameFilters(filters);
  dialog.setViewMode(QFileDialog::Detail);
...
void MainWindow::saveDecodedTextFileAsColumns(QString filename) {
  ...
  */
  ...
  if (CSVAbsTime) {
    builderString += tr("Year") + "," + tr("Month") + "," + tr("Day") + "," +
      ...
      if (msg != nullptr) {
        ...
        for (int j = 0; j < msg->sigHandler->getCount(); j++) {
          if (j == 0) {
            ...
            if (found == false)
              msgsAndColumns.append(
                  QPair<uint32_t, int>(msg->ID, columnsAdded));
          }

          if (found == false) {
            QString temp;
            if (msg->sigHandler->findSignalByIdx(j)->processAsText(*frame,
                                                                   temp)) {
  ...
  for (int c = 0; c < frames->count(); c++) {
    ...
    if (CSVAbsTime) {
      QDateTime dt = QDateTime::fromMSecsSinceEpoch(
          frame->timeStamp().microSeconds() / 1000);
      builderString += QString::number(dt.date().year()) + "," +
                       QString::number(dt.date().month()) + ",";
      builderString += QString::number(dt.date().day()) + "," +
                       QString::number(dt.time().hour()) + ",";
      builderString += QString::number(dt.time().minute()) + "," +
                       QString::number(dt.time().second()) + ",";
      builderString += QString::number(dt.time().msec()) + ",";
      dataColumnsAdded += 7;
    } else {
      builderString +=
          QString::number((frame->timeStamp().microSeconds() / 1000000.0), 'f',
                          6) +
          ",";
      dataColumnsAdded++;
    }
    // id
    builderString += Utility::formatCANID(frame->frameId(),
    ...
    if (dbcHandler != nullptr) {
      ...
      if (msg != nullptr) {
        for (int j = 0; j < msg->sigHandler->getCount(); j++) {
          ...
          QString temp;
          if (msg->sigHandler->findSignalByIdx(j)->processAsText(
                  *frame, temp, false, false)) {
...
void MainWindow::saveDecodedTextFile(QString filename) {
  ...
  */
  for (int c = 0; c < frames->count(); c++) {
    ...
    builderString +=
        tr("    ID: ") +
        Utility::formatCANID(frame->frameId(), frame->hasExtendedFrameFormat());
    if (frame->hasExtendedFrameFormat())
      builderString += tr(" Ext ");
    else
      builderString += tr(" Std ");
...
void MainWindow::connectionStatusUpdated(int conns) {
  lbStatusConnected.setText(tr("Connected to ") + QString::number(conns) +
                            tr(" buses"));
}
...
CANFrameModel *MainWindow::getCANFrameModel() { return model; }

/*
 * All functions past this point set up the various other windows that can be
 * opened
 */
...
void MainWindow::showSettingsDialog() {
  if (!settingsDialog) {
    ...
    connect(settingsDialog, SIGNAL(updatedSettings()), this,
            SLOT(readUpdateableSettings()));
  }
  settingsDialog->show();
}

// always gets unfiltered list. You ask for the graphs so there is no need to
// send filtered frames now always creates a new window. This allows for
// multiple independent graphing windows
void MainWindow::showGraphingWindow() {
  /* could only allow the latest window to have these centering signals.
     if (lastGraphingWindow)
      {
          disconnect(lastGraphingWindow,
     SIGNAL(sendCenterTimeID(uint32_t,double)), this,
     SLOT(gotCenterTimeID(int32_t,double))); disconnect(this,
     SIGNAL(sendCenterTimeID(uint32_t,double)), lastGraphingWindow,
     SLOT(gotCenterTimeID(int32_t,double))); if (flowViewWindow)
          {
              disconnect(lastGraphingWindow,
     SIGNAL(sendCenterTimeID(uint32_t,double)), flowViewWindow,
     SLOT(gotCenterTimeID(int32_t,double))); disconnect(flowViewWindow,
     SIGNAL(sendCenterTimeID(uint32_t,double)), lastGraphingWindow,
     SLOT(gotCenterTimeID(int32_t,double)));
          }
      }
  */
  ...
  connect(lastGraphingWindow, SIGNAL(sendCenterTimeID(uint32_t, double)), this,
          SLOT(gotCenterTimeID(uint32_t, double)));
  connect(this, SIGNAL(sendCenterTimeID(uint32_t, double)), lastGraphingWindow,
          SLOT(gotCenterTimeID(uint32_t, double)));
...
void MainWindow::showTemporalGraphWindow() {
  // only create an instance of the object if we dont have one. Otherwise just
  ...
  if (!temporalGraphWindow) {
    ...
    if (frames->count() > 2000) {
      ...
      confirmDialog = QMessageBox::question(
          this, "Danger Will Robinson",
          "There are a lot of frames (>2000) to plot, this may take a while or "
          "crash the app. Crash likely with more than 10k frames. Continue?",
          QMessageBox::Yes | QMessageBox::No);
...
void MainWindow::showISOInterpreterWindow() {
  if (!isoWindow) {
    if (!useFiltered)
      ...
      isoWindow =
          new ISOTP_InterpreterWindow(model->getFilteredListReference());
  }
  isoWindow->show();
}
...
void MainWindow::showFrameSenderWindow() {
  if (!frameSenderWindow) {
    if (!useFiltered)
      ...
      frameSenderWindow =
          new FrameSenderWindow(model->getFilteredListReference());
  }
...
void MainWindow::showPlaybackWindow() {
  if (!playbackWindow) {
    if (!useFiltered)
      ...
      playbackWindow =
          new FramePlaybackWindow(model->getFilteredListReference());
  }
  playbackWindow->show();
}
...
void MainWindow::showFirmwareUploaderWindow() {
  if (!firmwareUploaderWindow) {
    firmwareUploaderWindow =
        new FirmwareUploaderWindow(model->getListReference());
  }
...
void MainWindow::showMCConfigWindow() {
  if (!motorctrlConfigWindow) {
    motorctrlConfigWindow =
        new MotorControllerConfigWindow(model->getListReference());
    // connect(motorctrlConfigWindow, SIGNAL(sendCANFrame(const CANFrame*,int)),
    // worker, SLOT(sendFrame(const CANFrame*,int)));
    // connect(motorctrlConfigWindow, SIGNAL(sendFrameBatch(const
    // QList<CANFrame>*)), worker, SLOT(sendFrameBatch(const
    // QList<CANFrame>*)));
  }
...
void MainWindow::showFlowViewWindow() {
  if (!flowViewWindow) {
    ...
    connect(flowViewWindow, SIGNAL(sendCenterTimeID(uint32_t, double)), this,
            SLOT(gotCenterTimeID(int32_t, double)));
    connect(this, SIGNAL(sendCenterTimeID(uint32_t, double)), flowViewWindow,
            SLOT(gotCenterTimeID(int32_t, double)));
  }

  if (lastGraphingWindow) {
    connect(lastGraphingWindow, SIGNAL(sendCenterTimeID(uint32_t, double)),
            flowViewWindow, SLOT(gotCenterTimeID(int32_t, double)));
    connect(flowViewWindow, SIGNAL(sendCenterTimeID(uint32_t, double)),
            lastGraphingWindow, SLOT(gotCenterTimeID(int32_t, double)));
  }
...
void MainWindow::showDBCFileWindow() {
  if (!dbcFileWindow) {
    ...
    connect(dbcFileWindow, &DBCLoadSaveWindow::updatedDBCSettings, this,
            &MainWindow::DBCSettingsUpdated);
  }
  dbcFileWindow->show();
}
...
void MainWindow::showSignalViewer() {
  if (!signalViewerWindow) {
    if (!useFiltered)
      ...
      signalViewerWindow =
          new SignalViewerWindow(model->getFilteredListReference());
  }

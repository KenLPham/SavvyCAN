...
protected:
   ...
   * @param pDriver: string containing driver name - Really only used for
   * SerialBus connections
   * @param pType: the type of connection @ref CANCon::type
   * @param pSerialSpeed: for devices with variable serial speed this is that
   * speed.
   ...
   * @param pUseThread: if set to true, object will be execute in a dedicated
   * thread
   */
  CANConnection(QString pPort, QString pDriver, CANCon::type pType,
                int pSerialSpeed, int pBusSpeed, bool pCanFd, int pDataRate,
                int pNumBuses, int pQueueLen, bool pUseThread);
...
signals:
  ...
  void deviceInfo(int, int); // First param = driver version (or version of
                             // whatever you want), second param a status byte

  // bus number, bus speed, status (bit 0 = enabled, 1 = single wire, 2 = listen
  // only) 3 = Use value stored for enabled, 4 = use value passed for single
  // wire, 5 = use value passed for listen only 6 = use value passed for speed.
  // This allows bus status to be updated but set that some things aren't really
  ...
  /**
   * @brief event emitted when the CANCon::status of the connection changes
   * (connected->not_connected or the other way round)
   * @param pStatus: the new status of the device
   */
  void status(CANConStatus pStatus);

  /**
   * @brief Event sent when device has done something worthy of debugging
   * output.
...
public slots:
   ...
   * @note starts the working thread if required (piStarted is called in the
   * working thread context)
   */
  void start();

  /**
   * @brief stop the device. This calls piStop
   * @note if a working thread is used, piStop is called before exiting the
   * working thread
   */
  void stop();
   ...
   * @note this calls piSetBusSettings in the working thread context (if one has
   * been started)
   */
   ...
   * @param pBusIdx: the index of the bus for which settings have to be
   * retrieved
   * @param pBus: the CANBus struct to fill with information
   * @return true if operation succeeds, false if pBusIdx is invalid or bus has
   * not been configured yet
   * @note this calls piGetBusSettings in the working thread context (if one has
   * been started)
   */
   ...
   * @note this calls piSuspend (in the working thread context if one has been
   * started)
   ...
   * @note this calls piSendFrame (in the working thread context if one has been
   * started)
   */
   ...
   * @return false if parameter is invalid (bus id for instance)
   * @note this calls piSendFrameBatch (in the working thread context if one has
   * been started)
   */
  bool sendFrames(const QList<CANFrame> &pFrames);

  /**
   * @brief Add a new filter for the targetted frames. If a frame matches it
   * will immediately be sent via the targettedFrameReceived signal
   * @param pBusId - Which bus to bond to. -1 for any, otherwise a bitfield of
   * buses (but 0 = first bus, etc)
   * @param ID - 11 or 29 bit ID to match against
   * @param mask - 11 or 29 bit mask used for filter
   * @param receiver - Pointer to a QObject that wants to receive notification
   * when filter is matched
   * @return true if filter was able to be added, false otherwise.
   */
  bool addTargettedFrame(int pBusId, uint32_t ID, uint32_t mask,
                         QObject *receiver);

  /**
   * @brief Try to find a matching filter in the list and remove it, no longer
   * targetting those frames
   * @param pBusId - Which bus to bond to. Doesn't have to match the call to
   * addTargettedFrame exactly. You could disconnect just one bus for instance.
   * @param ID - 11 or 29 bit ID to match against
   * @param mask - 11 or 29 bit mask used for filter
   * @param receiver - Pointer to a QObject that wants to receive notification
   * when filter is matched
   ...
   */
  bool removeTargettedFrame(int pBusId, uint32_t ID, uint32_t mask,
                            QObject *receiver);
...
protected:
  int mNumBuses; // protected to allow connected device to figure out how many
                 // buses are available
   ...
   * @param pBusId
   * @param pConfigured
   * @note it is not necessary to call this function to set pBusId configured,
   * it is enough to call @ref setBusConfig
   */
...
protected:
   ...
   * @brief suspends/restarts data capture
   * @param pSuspend: suspends capture if true else restarts it
   * @note the caller will not access the queue when capture is suspended, so it
   * is safe for callee to flush the queue
   */

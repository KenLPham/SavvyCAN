...
CANConnection::CANConnection(QString pPort, QString pDriver, CANCon::type pType,
                             int pSerialSpeed, int pBusSpeed, bool pCanFd,
                             int pDataRate, int pNumBuses, int pQueueLen,
                             bool pUseThread)
    : mNumBuses(pNumBuses), mSerialSpeed(pSerialSpeed), mQueue(), mPort(pPort),
      mDriver(pDriver), mType(pType), mIsCapSuspended(false),
      mStatus(CANCon::NOT_CONNECTED), mStarted(false), mThread_p(nullptr) {
  /* register types */
  ...
  if (pBusSpeed > 0)
    mBusData[0].mBus.setSpeed(pBusSpeed);
...
void CANConnection::suspend(bool pSuspend) {
  ...
  if (mThread_p && (mThread_p != QThread::currentThread())) {
    QMetaObject::invokeMethod(this, "suspend", Qt::BlockingQueuedConnection,
                              Q_ARG(bool, pSuspend));
    return;
  }
...
void CANConnection::stop() {
  ...
  if (mThread_p && mStarted && (mThread_p != QThread::currentThread())) {
    /* if thread is finished, it means we call this function for the second time
     * so we can leave */
...
bool CANConnection::getBusSettings(int pBusIdx, CANBus &pBus) {
  ...
  if (mThread_p && (mThread_p != QThread::currentThread())) {
    ...
    QMetaObject::invokeMethod(
        this, "getBusSettings", Qt::BlockingQueuedConnection,
        Q_RETURN_ARG(bool, ret), Q_ARG(int, pBusIdx), Q_ARG(CANBus &, pBus));
    return ret;
  }
...
void CANConnection::setBusSettings(int pBusIdx, CANBus pBus) {
  ...
  if (mThread_p && (mThread_p != QThread::currentThread())) {
    QMetaObject::invokeMethod(this, "setBusSettings",
                              Qt::BlockingQueuedConnection, Q_ARG(int, pBusIdx),
                              Q_ARG(CANBus, pBus));
    return;
  }
...
bool CANConnection::sendFrame(const CANFrame &pFrame) {
  ...
  if (mThread_p && (mThread_p != QThread::currentThread())) {
    bool ret;
    QMetaObject::invokeMethod(this, "sendFrame", Qt::BlockingQueuedConnection,
...
bool CANConnection::sendFrames(const QList<CANFrame> &pFrames) {
  ...
  if (mThread_p && (mThread_p != QThread::currentThread())) {
    bool ret;
    QMetaObject::invokeMethod(this, "sendFrames", Qt::BlockingQueuedConnection,
                              Q_RETURN_ARG(bool, ret),
...
int CANConnection::getNumBuses() const { return mNumBuses; }
...
QString CANConnection::getPort() { return mPort; }

QString CANConnection::getDriver() { return mDriver; }

LFQueue<CANFrame> &CANConnection::getQueue() { return mQueue; }

CANCon::type CANConnection::getType() { return mType; }
...
bool CANConnection::isCapSuspended() { return mIsCapSuspended; }

void CANConnection::setCapSuspended(bool pIsSuspended) {
  mIsCapSuspended = pIsSuspended;
}

void CANConnection::debugInput(QByteArray bytes) { Q_UNUSED(bytes) }

bool CANConnection::addTargettedFrame(int pBusId, uint32_t ID, uint32_t mask,
                                      QObject *receiver) {
  /*
      if( mThread_p && (mThread_p != QThread::currentThread()) ) {
          bool ret;
  ...
  */
  ...
  qDebug()
      << "Connection is registering a new targetted frame filter, local bus "
      << pBusId;
  ...
  if (pBusId > -1)
    ...
    for (int i = 0; i < mBusData.count(); i++)
      mBusData[i].mTargettedFrames.append(target);
  }

  return true;
}

bool CANConnection::removeTargettedFrame(int pBusId, uint32_t ID, uint32_t mask,
                                         QObject *receiver) {
  /*
      if( mThread_p && (mThread_p != QThread::currentThread()) ) {
          bool ret;
...
bool CANConnection::removeAllTargettedFrames(QObject *receiver) {
  for (int i = 0; i < getNumBuses(); i++) {
    foreach (const CANFltObserver filt, mBusData[i].mTargettedFrames) {
      if (filt.observer == receiver)
        mBusData[i].mTargettedFrames.removeOne(filt);
    }
  }
...
void CANConnection::checkTargettedFrame(CANFrame &frame) {
  ...
  int bus = frame.bus;
  if (bus > (mBusData.length() - 1))
    bus = mBusData.length() - 1;

  if (mBusData[bus].mTargettedFrames.length() == 0)
    return;
  foreach (const CANFltObserver filt, mBusData[frame.bus].mTargettedFrames) {
    // qDebug() << "Checking filter with id " << filt.id << " mask " <<
    // filt.mask;
    maskedID = frame.frameId() & filt.mask;
    if (maskedID == filt.id) {
      qDebug()
          << "In connection object I got a targetted frame. Forwarding it.";
      QMetaObject::invokeMethod(filt.observer, "gotTargettedFrame",
                                Qt::QueuedConnection, Q_ARG(CANFrame, frame));
    }
  }
}

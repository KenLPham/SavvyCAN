...
CANFrameModel::CANFrameModel(QObject *parent) : QAbstractTableModel(parent) {
  ...
  } else // if compiling for 32 bit you can't ask for gigabytes of preallocation
         // so tone it down.
  {
  ...
  // Each CANFrame object takes up 56 bytes and we're allocating two arrays here
  // so take the # of pre-alloc frames and multiply by 112 to get the RAM usage.
  // This is around 1GiB for the default.

  // the goal is to prevent a reallocation from ever happening
  frames.reserve(preallocSize);
  // this is pretty wasteful. We're storing all frames twice. It may be better
  // for filteredFrames to be a list of pointers. Pointers take up 8 bytes
  // instead of 56 so this is quite a savings for RAM usage. But, then
  // filteredFrames would work differently from frames above so the two could
  // not be used interchangeably. Still think about what can be done.
...
void CANFrameModel::setBytesPerLine(int bpl) { bytesPerLine = bpl; }
...
bool CANFrameModel::getInterpretMode() { return interpretFrames; }
...
/*
 * Scan all frames for the smallest timestamp and offset all timestamps so that
 * smallest one is at 0
 */
void CANFrameModel::normalizeTiming() {
  ...
  // find the absolute lowest timestamp in the whole time. Needed because maybe
  // timestamp was reset in the middle.
  for (int j = 0; j < frames.count(); j++) {
    if (frames[j].timeStamp().microSeconds() < timeOffset)
      timeOffset = frames[j].timeStamp().microSeconds();
  }
  ...
  for (int i = 0; i < filteredFrames.count(); i++) {
    filteredFrames[i].setTimeStamp(QCanBusFrame::TimeStamp(
        0, filteredFrames[i].timeStamp().microSeconds() - timeOffset));
  }
  this->endResetModel();
...
void CANFrameModel::setHideTx(bool mode) {
  hideTx = mode;
  sendRefresh();
}
...
/*
 * There is probably a more correct way to have done this but below are several
 * functions that collectively implement quicksort on the columns and interpret
 * the columns numerically. But, correct or not, this implementation is quite
 * fast and sorts the columns properly.
 */
uint64_t CANFrameModel::getCANFrameVal(QVector<CANFrame> *frames, int row,
                                       Column col) {
  uint64_t temp = 0;
  if (row >= frames->count())
    return 0;
  ...
  case Column::Data:
    for (int i = 0; i < std::min(frame.payload().length(), 8); i++)
...
void CANFrameModel::qSortCANFrameAsc(QVector<CANFrame> *frames, Column column,
                                     int lowerBound, int upperBound) {
  int p, i, j;
  qDebug() << "Lower " << lowerBound << " Upper" << upperBound;
  if (lowerBound < upperBound) {
    uint64_t piv = getCANFrameVal(
        frames, lowerBound + (upperBound - lowerBound) / 2, column);
    i = lowerBound - 1;
    j = upperBound + 1;
    for (;;) {
      do {
...
void CANFrameModel::qSortCANFrameDesc(QVector<CANFrame> *frames, Column column,
                                      int lowerBound, int upperBound) {
  int p, i, j;
  qDebug() << "Lower " << lowerBound << " Upper" << upperBound;
  if (lowerBound < upperBound) {
...
void CANFrameModel::sortByColumn(int column) {
  ...
  if (sortDirAsc)
    qSortCANFrameAsc(&filteredFrames, Column(column), 0,
                     filteredFrames.count() - 1);
  else
    qSortCANFrameDesc(&filteredFrames, Column(column), 0,
                      filteredFrames.count() - 1);
...
void CANFrameModel::recalcOverwrite() {
  if (!overwriteDups)
    return; // no need to do a thing if mode is disabled
  ...
  // Look at the current list of frames and turn it into just a list of unique
  // IDs
...
QVariant CANFrameModel::data(const QModelIndex &index, int role) const {
  ...
  const unsigned char *data =
      reinterpret_cast<const unsigned char *>(thisFrame.payload().constData());
  ...
  if (role == Qt::DisplayRole) {
    ...
    case Column::TimeStamp:
      ...
      if (overwriteDups) {
        if (timeStyle == TS_SECONDS)
          return QString::number(thisFrame.timedelta / 1000000.0, 'f', 5);
        return QString::number(thisFrame.timedelta);
      } else
        ts = Utility::formatTimestamp(thisFrame.timeStamp().microSeconds());
      if (ts.type() == QVariant::Double)
        return QString::number(
            ts.toDouble(), 'f',
            5); // never scientific notation, 5 decimal places
      if (ts.type() == QVariant::LongLong)
        return QString::number(
            ts.toLongLong()); // never scientific notion, all digits shown
      if (ts.type() == QVariant::DateTime)
        return ts.toDateTime().toString(
            timeFormat); // custom set format for dates and times
      return Utility::formatTimestamp(thisFrame.timeStamp().microSeconds());
    case Column::FrameId:
      return Utility::formatCANID(thisFrame.frameId(),
                                  thisFrame.hasExtendedFrameFormat());
    case Column::Extended:
      return QString::number(thisFrame.hasExtendedFrameFormat());
    case Column::Remote:
      if (!overwriteDups)
        return QString::number(thisFrame.frameType() ==
                               QCanBusFrame::RemoteRequestFrame);
      return QString::number(thisFrame.frameCount);
    case Column::Direction:
      if (thisFrame.isReceived)
        return QString(tr("Rx"));
    ...
    case Column::ASCII:
      ...
      if (thisFrame.frameType() == QCanBusFrame::DataFrame) {
        ...
        for (int i = 0; i < dataLen; i++) {
          ...
          // 0x20 through 0x7E are printable characters. Outside of that range
          // they aren't. So use dots instead
          if (byt < 0x20)
            byt = 0x2E; // dot character
          if (byt > 0x7E)
            byt = 0x2E;
    ...
    case Column::Data:
      ...
      if (thisFrame.frameType() == thisFrame.ErrorFrame) {
        if (thisFrame.error() & thisFrame.TransmissionTimeoutError)
          tempString.append("\nTX Timeout");
        if (thisFrame.error() & thisFrame.LostArbitrationError)
          tempString.append("\nLost Arbitration");
        if (thisFrame.error() & thisFrame.ControllerError)
          tempString.append("\nController Error");
        if (thisFrame.error() & thisFrame.ProtocolViolationError)
          tempString.append("\nProtocol Violation");
        if (thisFrame.error() & thisFrame.TransceiverError)
          tempString.append("\nTransceiver Error");
        if (thisFrame.error() & thisFrame.MissingAcknowledgmentError)
          tempString.append("\nMissing ACK");
        if (thisFrame.error() & thisFrame.BusOffError)
          tempString.append("\nBus OFF");
        if (thisFrame.error() & thisFrame.BusError)
          tempString.append("\nBus ERR");
        if (thisFrame.error() & thisFrame.ControllerRestartError)
          tempString.append("\nController restart err");
        if (thisFrame.error() & thisFrame.UnknownError)
          tempString.append("\nUnknown error type");
      }
      // TODO: technically the actual returned bytes for an error frame encode
      // some more info. Not interpreting it yet.

      // now, if we're supposed to interpret the data and the DBC handler is
      // loaded then use it
      if ((dbcHandler != nullptr) && interpretFrames &&
          (thisFrame.frameType() == thisFrame.DataFrame)) {
        DBC_MESSAGE *msg = dbcHandler->findMessage(thisFrame);
        if (msg != nullptr) {
          ...
          for (int j = 0; j < msg->sigHandler->getCount(); j++) {
            ...
            if ((sig->multiplexParent == nullptr) &&
                sig->processAsText(thisFrame, sigString)) {
              ...
              if (sig->isMultiplexor) {
                ...
                tempString.append(sig->processSignalTree(thisFrame));
                       ...
                       overwriteDups) // wasn't in this exact frame but is in
                                      // the message. Use cached value
            {
              bool isInteger = false;
              if (sig->valType == UNSIGNED_INT || sig->valType == SIGNED_INT)
                isInteger = true;
              tempString.append(sig->makePrettyOutput(
                  sig->cachedValue.toDouble(), sig->cachedValue.toLongLong(),
                  true, isInteger));
              tempString.append("\n");
            }
          }
...
void CANFrameModel::addFrame(const CANFrame &frame, bool autoRefresh = false) {
  ...
  tempFrame.setTimeStamp(QCanBusFrame::TimeStamp(
      0, tempFrame.timeStamp().microSeconds() - timeOffset));

  lastUpdateNumFrames++;

  // if this ID isn't found in the filters list then add it and show it by
  // default
  if (!filters.contains(tempFrame.frameId())) {
    // if there are any filters already configured, leave the new filter
    // disabled
  ...
  // if this BusID isn't found in the busFilters list then add it and show it by
  // default
  if (!busFilters.contains(tempFrame.bus)) {
    // if there are any busFilters already configured, leave the new filter
    // disabled
  ...
  if (!overwriteDups) {
    try {
      ...
      if (filters[tempFrame.frameId()] && busFilters[tempFrame.bus] &&
          (tempFrame.isReceived || (!tempFrame.isReceived && !hideTx))) {
        if (autoRefresh)
          beginInsertRows(QModelIndex(), filteredFrames.count(),
                          filteredFrames.count());
      ...
      qDebug() << "addFrame failed to append. App is probably going to crash. "
                  "frames.length(): "
               << frames.length() << " Exception: " << ex.what();
    }
  } else // yes, overwrite dups
  {
    ...
    //        for (int i = 0; i < frames.count(); i++)
    //        {
    //            if ( (frames[i].frameId() == tempFrame.frameId()) &&
    //            (frames[i].bus == tempFrame.bus) )
    //            {
    //                tempFrame.frameCount = frames[i].frameCount + 1;
    //                tempFrame.timedelta = tempFrame.timeStamp().microSeconds()
    //                - frames[i].timeStamp().microSeconds(); frames.replace(i,
    //                tempFrame); found = true; break;
    //            }
    //        }
    for (int i = 0; i < filteredFrames.count(); i++) {
...
void CANFrameModel::addFrames(const CANConnection *,
                              const QVector<CANFrame> &pFrames) {
  if (frames.length() > frames.capacity() * 0.99) {
    mutex.lock();
    qDebug() << "Frames count: " << frames.length() << " of "
             << frames.capacity() << " capacity, removing first "
             << (int)(frames.capacity() * 0.05) << " frames";
  ...
  if (filteredFrames.length() > filteredFrames.capacity() * 0.99) {
    mutex.lock();
    qDebug() << "filteredFrames count: " << filteredFrames.length() << " of "
             << filteredFrames.capacity() << " capacity, removing first "
             << (int)(filteredFrames.capacity() * 0.05) << " frames";
    filteredFrames.remove(0, (int)(filteredFrames.capacity() * 0.05));
    qDebug() << "filteredFrames removed, new count: "
  ...
  if (overwriteDups) // if in overwrite mode we'll update every time frames come
                     // in
  {
    beginResetModel();
    endResetModel();
...
void CANFrameModel::sendRefresh() {
  ...
  if (overwriteDups) {
    ...
    for (int i = 0; i < count; i++) {
      if (filters[frames[i].frameId()] && busFilters[frames[i].bus] &&
          (frames[i].isReceived || (!frames[i].isReceived && !hideTx))) {
...
int CANFrameModel::sendBulkRefresh() {
  ...
  if (lastUpdateNumFrames <= 0)
    return 0;

  if (lastUpdateNumFrames == 0 && !overwriteDups)
    return 0;
...
void CANFrameModel::insertFrames(const QVector<CANFrame> &newFrames) {
  // not resetting the model here because the serial worker automatically does a
  // bulk refresh every 1/4 second and that refresh will cause the view to
  // update. If you do both it usually ends up thinking you have double the
  // number of frames. beginResetModel();
  mutex.lock();
  ...
  for (int i = 0; i < newFrames.count(); i++) {
    ...
    if (filters[newFrames[i].frameId()] && busFilters[newFrames[i].bus] &&
        (newFrames[i].isReceived || (!newFrames[i].isReceived && !hideTx))) {
      insertedFiltered++;
  ...
  mutex.unlock();
  // endResetModel();
  // beginInsertRows(QModelIndex(), filteredFrames.count() + 1,
  // filteredFrames.count() + insertedFiltered); endInsertRows();
  ...
  while (!inFile->atEnd()) {
    ...
    if (line.length() > 2) {
      ...
      if (tokens[1].toUpper() == "T")
        filters.insert(ID, true);
      else
        filters.insert(ID, false);
    }
  }
  inFile->close();

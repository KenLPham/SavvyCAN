...
public slots:
  ...
  void updateConnectionSettings(QString connectionType, QString port,
...
signals:
  ...
  //-1 = frames cleared, -2 = a new file has been loaded (so all frames are
  //different), otherwise # of new frames
  void framesUpdated(int numFrames); // something has updated the frame list
                                     // (send at gui update frequency)
...
private:
  ...
  bool useFiltered; // should sub-windows use the unfiltered or filtered frames
                    // list?
  ...
  // Graph window is allowed to instantiate more than once. All the rest are not
  // (yet).
